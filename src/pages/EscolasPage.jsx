...
export default function EscolasPage() {
  const { escolas: escolasData } = useData();
  ...
  const allCountries = useMemo(() => {
    const set = new Set(escolasData.map((s) => s.pais).filter(Boolean));
    return [...set].sort();
  }, [escolasData]);

  const allAreas = useMemo(() => extractUniqueAreas(escolasData), [escolasData]);

  const filtered = useMemo(() => {
    return escolasData.filter((item) => {
      ...
      const matchSearch =
        ...
        (item.relevancia && item.relevancia.toLowerCase().includes(q));
  ...
  // Quick-filter areas (most common)
  const quickAreas = [
    'Engenharia', 'TI', 'Saúde', 'Manufatura', 'Construção Civil',
    'Mecânica', 'Automotivo', 'Energia', 'Agropecuária', 'Gestão',
    'Multissetorial',
  ];
    ...
    <Box sx={{ maxWidth: 1400, mx: 'auto', px: { xs: 2, md: 3 }, py: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h5" fontWeight={700} color="primary.main">
          Escolas &amp; Institutos Internacionais
        </Typography>
      ...
      <Grid container spacing={2.5} sx={{ mt: 1 }}>
        {filtered.map((item) => (
          <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={item.id}>
            <EscolaCard item={item} onClick={() => setSelectedItem(item)} />
          </Grid>
        ))}
      </Grid>

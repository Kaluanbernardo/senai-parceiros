...
3. recomenda de 5 a 10 pessoas físicas ou pessoas jurídicas já cadastradas, com filtro por subtipo;
...
- identificadores estáveis para deduplicação (perfil profissional, LinkedIn e, quando acadêmico, Scholar/ORCID/OpenAlex para pessoas; domínio/identificador público para escolas e organizações).

Esses campos devem ser representados no contrato versionado e mapeados para os campos canônicos consumidos por `selectionEngine`, `schoolCatalog` e pelo normalizador legado de pessoas em `researcherCatalog`. Se uma coluna nova for necessária, ela deve ser criada no schema compartilhado e refletida ao mesmo tempo no prompt, no template, no parser, na prévia, no catálogo e nos testes de round-trip.
...
- catálogo unificado em pessoas físicas e pessoas jurídicas, com subtipos filtráveis;
...
- pessoas sem fotos, avatares, iniciais ou placeholders de mídia;
...
- A primeira escolha é sempre: pessoa física ou pessoa jurídica; o subtipo é definido em seguida.
...
- Exemplos devem usar a natureza, o subtipo, o objetivo e o contexto atuais; uma instituição de ensino para benchmarking não recebe exemplo de palestra sobre IA.
...
4. [Canonizar e deduplicar pessoas e instituições de educação](luna-v3/03-catalogos-canonicos.md) — bloqueado pelo baseline; pode avançar em paralelo com a entrevista. A próxima revisão deve cobrir duplicatas semânticas residuais por identificador, domínio e variantes multilíngues.
...
- nenhuma pessoa volta a exibir foto, avatar, iniciais ou placeholder de mídia.
